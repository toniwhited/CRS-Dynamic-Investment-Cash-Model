...
## Plotting (`plot_policies.jl`)

Julia script to generate surface plots of the value and policy functions.

**Requirements:** Julia with `DelimitedFiles` and `Plots` packages.

```bash
julia plot_policies.jl
```

Reads `v.txt`, `c.txt`, `i.txt`, `cg.txt`, and `zg.txt` from `Outputcomp/` and produces:
- `value_function.png`: Value function V(c,z)
- `cash_policy.png`: Cash policy c'(c,z)
- `investment_policy.png`: Investment policy i(c,z)
